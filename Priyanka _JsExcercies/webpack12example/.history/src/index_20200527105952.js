...
//   var address = document.getElementById('address').value;
  var person = new Person(domAccess.getName(), domAccess.getAge(), domAccess.getAddress());
...
export function getByAgeCondition() {
  ...
  var ageCondition = domAccess.getAgeCondition();
  ...
  var personsList = "";
  for(const person of tempList) { //This is ES6 for loop
    personsList += person.describe();
  }

  //4. update personsList
  var listElement = domAccess.updatePersonsList(personsList);
}

//Please fill code
export function getBySearchCondition() {
  var searchvalue = domAccess.getSearchCriteria();
  //2. if Age is zero get all persons description
  var tempList = Persons;
