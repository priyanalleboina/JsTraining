...
export function getPersonDetails() {
  ...
  //let arradata = ContactsArray;
  if (pid.length != 0) {
 ContactsArray = ContactsArray.filter(function(pid1) {
      return pid1.address.street.match(pid);
    });
    console.log(ContactsArray);
  }
  // ContactsArray += contact1.describe(); 
  // let data = arradata;
  let varable = '';
  for (const data1 of ContactsArray) {
    varable += data1.describe();
  }
  let listElement = document.getElementById("personsDetails");
  listElement.innerHTML = varable;
